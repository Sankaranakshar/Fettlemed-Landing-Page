  ...
  return (
    ...
    >
      <div className="bg-white rounded-[1.5rem] border border-stone-200 shadow-xl overflow-hidden">
        ...
        {/* Sidebar + main layout */}
        <div className="flex flex-col sm:flex-row min-h-0 sm:min-h-[270px]">
          {/* Sidebar */}
          <div className="w-full sm:w-36 border-b sm:border-b-0 sm:border-r border-stone-100 bg-stone-50 flex flex-row sm:flex-col py-2 sm:py-3 gap-1 shrink-0 overflow-x-auto">
            {views.map((v) => {
            ...
            <div className="hidden sm:flex sm:mt-auto px-4 pb-2">
              <div className="flex items-center gap-1.5">
                <Users className="w-3 h-3 text-stone-400" />
                <span className="text-[9px] text-stone-400 font-medium">12 patients today</span>
              </div>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="appts" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.25 }} className="space-y-2">
                  ...
                  {[
                    ...
                    <motion.div key={name} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-center gap-3 bg-stone-50 rounded-xl px-3 py-2.5">
                      <div className="w-7 h-7 rounded-full bg-pine-100 flex items-center justify-center text-[10px] font-medium text-pine-700 shrink-0">{name[0]}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-pine-900 truncate">{name}</p>
              ...
              {active === "Billing" && (
                <motion.div key="billing" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.25 }} className="space-y-3">
                  <p className="text-[10px] font-medium text-stone-400 uppercase tracking-wider mb-3">Today's collections</p>
                  <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-pine-900 rounded-2xl p-4 text-white">
                    ...
                    <p className="text-2xl font-medium">₹14,800</p>
                  ...
                  {[
                    ...
                    <motion.div key={label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.1 }} className="flex justify-between items-center py-1.5 border-b border-stone-100 last:border-0">
                      ...
                      <span className="text-xs font-medium text-pine-900">{amount}</span>
                    </motion.div>
                  ))}
                </motion.div>
              )}
              ...
              {active === "Reports" && (
                <motion.div key="reports" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.25 }} className="space-y-3">
                  <p className="text-[10px] font-medium text-stone-400 uppercase tracking-wider mb-3">This week</p>
                  {[
                    ...
                    <motion.div key={label} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="space-y-1.5">
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-stone-500 font-medium">{label}</span>
                        <span className="text-xs font-medium text-pine-900">{value}{typeof value === "number" && max ? `/${max}` : ""}</span>
                      </div>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="flex items-center gap-2 bg-green-50 border border-green-100 rounded-xl px-3 py-2 mt-2">
                    ...
                    <p className="text-[10px] font-medium text-green-700">Peak hours: 9–11 AM. Consider adding a slot.</p>
                  </motion.div>
                </motion.div>
              )}
            </AnimatePresence>
