    ...
    >
      <div className="bg-white rounded-[1.5rem] border border-stone-200 shadow-xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-3 bg-pine-900 text-white">
          <div className="w-8 h-8 rounded-full bg-pine-700 flex items-center justify-center font-medium text-sm">R</div>
          <div>
            <p className="text-sm font-medium">Rajan Subramaniam</p>
          ...
          <div className="ml-auto flex items-center gap-1.5 bg-accent-600/30 border border-accent-500/30 px-3 py-1 rounded-full">
            ...
            <span className="text-xs font-medium text-accent-300">Consent granted</span>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="summary" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.25 }} className="space-y-3">
                  ...
                  {[
                    ...
                    <div key={label} className="bg-stone-50 rounded-xl p-3 flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-pine-900">{value}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-medium text-amber-700">Patient uploaded new CBC report today</p>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {active === "Prescriptions" && (
              <motion.div key="rx" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.25 }} className="space-y-3">
                <p className="text-[10px] font-medium text-stone-400 uppercase tracking-wider">Previous prescriptions</p>
                {[
                  ...
                  <div key={drug} className="flex items-center justify-between bg-stone-50 rounded-xl px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-medium text-pine-900">{drug}</p>
                        <p className="text-[10px] text-stone-400">{date}</p>
                      </div>
                    </div>
                    <span className={`text-[10px] font-medium px-2 py-1 rounded-full ${status === "Active" ? "bg-green-100 text-green-700" : "bg-stone-200 text-stone-500"}`}>{status}</span>
                  </div>
                ))}
                <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="w-full mt-2 py-2.5 rounded-xl border-2 border-dashed border-pine-200 text-pine-700 text-xs font-medium hover:bg-pine-50 transition-colors flex items-center justify-center gap-2">
            ...
            {active === "Add Note" && (
              <motion.div key="note" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.25 }} className="space-y-3">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0 }} className="flex gap-2">
                  ...
                  <button className="px-4 py-2.5 border border-stone-200 text-stone-600 rounded-xl text-xs font-medium hover:bg-stone-50 transition-colors">Cancel</button>
                </motion.div>
              </motion.div>
            )}
          </AnimatePresence>
